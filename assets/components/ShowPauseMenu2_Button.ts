...
@ccclass('ShowPauseMenu2_Button')
export class ShowPauseMenu2_Button extends Component {
    @property({ displayName: "暂停菜单节点" })
    public PauseMenu2Node: Node | null = null

    @property({ displayName: "退出游戏节点" })
    public exitGameNode: Node | null = null

    @property({ displayName: "存档游戏节点" })
    public saveGameNode: Node | null = null
    ...
    onButtonClick() {
        ...
        this.playClickAnimation(() => {
            director.emit("SHOW_PAUSE_MENU2")//触发显示暂停菜单事件(待定具体事件名称)
            console.log('已点击ShowPauseMenu2_Button')
        })
    }
