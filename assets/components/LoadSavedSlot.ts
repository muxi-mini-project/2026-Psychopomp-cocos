...
@ccclass('LoadSavedSlot')
export class LoadSavedSlot extends Component {
    ...
    onLoad() {
        ...
        this.node.on('click', this.onLoadSavedSlotClick, this)
    }

    onLoadSavedSlotClick() {
        this.playClickSound()
        this.playClickAnimation(() => {
            director.emit("LOAD_GAME", "auto_save")//加载自动存档
            console.log('已点击LoadSavedSlot_Button')
        })
    }
    ...
    onDestroy() {
        ...
        this.node.off('click', this.onLoadSavedSlotClick, this)
    }
}
